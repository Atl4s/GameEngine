...
{
	int i = 0;
	while (i < v_pGameObjects.size() && m_bStateIsOver!=true)
	{
		v_pGameObjects[i]->update();
		++i;
	}
	i = 0;
	while (i < v_pParticles.size() && m_bStateIsOver != true)
	{
...
{
	std::cout << s_gameOverID << std::endl;

	StateParser stateParser;
	stateParser.parseState("../assets/test.xml", s_gameOverID, &v_pGameObjects);
...
void GameOverState::initShaders()
{
